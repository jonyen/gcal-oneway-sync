...
// Normalize origin key by removing @google.com suffix from iCalUID
function normalizeOrigin(origin) {
    if (!origin)
        return origin;
    // Split by the last colon to separate the iCalUID part
    const parts = origin.split(':');
    if (parts.length < 2)
        return origin;
    // Get the last part (iCalUID) and remove @google.com if present
    const lastPart = parts[parts.length - 1];
    const normalizedLastPart = lastPart.replace(/@google\.com$/, '');
    // Reconstruct the origin with normalized iCalUID
    return [...parts.slice(0, -1), normalizedLastPart].join(':');
}
async function advancedDedupe() {
    const calendar = cal(TARGET_TOKENS);
    let pageToken;
    const groups = {};
    console.log(`Scanning target calendar ${TARGET_CALENDAR_ID} for duplicates with advanced normalization…`);
    do {
        ...
        for (const ev of data.items ?? []) {
            ...
            groups[normalizedOrigin] = groups[normalizedOrigin] || [];
            groups[normalizedOrigin].push(ev);
        }
        pageToken = data.nextPageToken || undefined;
    } while (pageToken);
    let deleted = 0, kept = 0;
    for (const [normalizedOrigin, events] of Object.entries(groups)) {
        if (events.length <= 1)
            continue;
        ...
        // Show original origins for debugging
        for (const event of events) {
            const originalOrigin = event.extendedProperties?.private?.origin;
            console.log(`  - ${event.id}: ${originalOrigin}`);
        }
    ...
    if (DRY_RUN) {
        ...
        console.log(`Advanced deduplication complete. Kept ${kept}, deleted ${deleted}.`);
    }
}
advancedDedupe().catch(err => {
    console.error("Advanced deduper failed:", err);
    process.exit(1);
});
//# sourceMappingURL=advanced-dedupe.js.map