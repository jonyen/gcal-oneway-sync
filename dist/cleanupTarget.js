...
async function run() {
    ...
    do {
        ...
        for (const ev of data.items ?? []) {
            seen++;
            const origin = ev.extendedProperties?.private?.[ORIGIN_KEY];
            if (!origin)
                continue;
            ...
            if (SOURCE_IDS.some(src => origin.startsWith(`${src}:`))) {
                try {
                    await target.events.delete({ calendarId: OLD_TARGET_CAL_ID, eventId: ev.id, sendUpdates: "none" });
                    deleted++;
                    if (deleted % 50 === 0)
...
//# sourceMappingURL=cleanupTarget.js.map